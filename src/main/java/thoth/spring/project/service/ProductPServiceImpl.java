...
@Service("pPsrv")
public class ProductPServiceImpl implements ProductService{

    @Autowired private ProductDAO pPdao;
    ...
    public List<Product> readProduct(String cp) {
        ...
        return pPdao.selectProduct(snum);
    }
    // 상품 조회 - 전체 상품 갯수 구하기
    @Override
    public int countProduct(){
        return pPdao.selectCountProduct();
    }
    ...
    public List<Product> readProduct(String cp, String ftype, String fkey) {
        ...
        return pPdao.findSelectProduct(params);
    }
    ...
    public int countProduct(String ftype, String fkey) {
        ...
        params.put("ftype",ftype);
        params.put("fkey",fkey);
        return pPdao.selectCountProduct(params);
    }

    // 상품 상세 조회
    @Override
    public Product readOneProduct(String tnum) {
        return pPdao.selectOneProduct(tnum);
    }
    ...
    public boolean newProduct(Product p, BookImage b, MultipartFile[] img){
        ...
        if(pPdao.insertProduct(p,b)>0) isInserted = true;
        //pPdao.insertProduct(p,b);
        return isInserted;
    }
    ...
    public Product removeProduct(String tnum) {
        Product p = pPdao.selectOneProduct(tnum); // 지우고자 하는 상품 분류코드
        pPdao.deleteProduct(tnum);
        return p;
    }
    ...
    public BookImage readOneImage(String tnum) {
        return pPdao.selectOneImage(tnum);
    }

    // 상품 삭제 - 이미지
    @Override
    public BookImage removeImage(String tnum) {
        BookImage b = pPdao.selectOneImage(tnum); // 지우고자 하는 상품 분류코드
        pPdao.deleteImage(tnum);
        return b;
    }
    ...
    public void modifyImage(BookImage b, MultipartFile[] img) {
        ...
        if(imgutil.checkImageFiles(img)) {
            ...
            String fn = pPdao.readFnames(b.getTnum());
            String fs = pPdao.readFsizes(b.getTnum());
