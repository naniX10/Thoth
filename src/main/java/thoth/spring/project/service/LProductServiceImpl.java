...
import thoth.spring.project.dao.LProductDAO;
...
@Service("plsrv")
public class LProductServiceImpl implements LProductService{

    @Autowired private LProductDAO pldao;
    @Autowired private ImgUploadUtil imgutil;

    /* ch */

    @Override
    public List<Product> lreadProduct(String cp) {
        int snum = (Integer.parseInt(cp)-1)*20; // 10개 게시물 출력
        return pldao.lselectProduct(snum);
    }

    @Override
    public int lcountProduct() {
        return pldao.lselectCountProduct();
    }

    @Override
    public List<Product> lreadProduct(String cp, String findtype, String findkey) {
        int snum = 10*(Integer.parseInt(cp)-1); // 10개 게시물 고정
        Map<String, Object> params = new HashMap<>();
        params.put("snum",snum);
        params.put("ftype",findtype);
        params.put("fkey",findkey);
        return pldao.lfindSelectProduct(params);
    }

    @Override
    public int lcountProduct(String findtype, String findkey) {
        Map<String,Object> params = new HashMap<>();
        params.put("ftype",findtype);
        params.put("fkey",findkey);
        return pldao.lselectCountProduct(params);
    }

    @Override
    public boolean lnewProduct(Product p, BookImage b, MultipartFile[] img) {
        // 식별코드 생성
        ...
        if(pldao.linsertProduct(p,b)>0) isInserted = true;
        //pdao.insertProduct(p,b);
        return isInserted;
    }

    @Override
    public Product lreadOneProduct(String tnum) {
        return pldao.lselectOneProduct(tnum);
    }

    @Override
    public BookImage lreadOneImage(String tnum) {
        return pldao.lselectOneImage(tnum);
    }

    @Override
    public Product lremoveProduct(String tnum) {
        Product p = pldao.lselectOneProduct(tnum); // 지우고자 하는 상품 분류코드
        pldao.ldeleteProduct(tnum);
        return p;
    }

    @Override
    public BookImage lremoveImage(String tnum) {
        BookImage b = pldao.lselectOneImage(tnum); // 지우고자 하는 상품 분류코드
        pldao.ldeleteImage(tnum);
        return b;
    }

    @Override
    public void lmodifyProduct(Product p) {
        pldao.lupdateProduct(p);
    }

    @Override
    public void lmodifyImage(BookImage b, MultipartFile[] img) {
        ...
        if(imgutil.checkImageFiles(img)) {
            ...
            String fn = pldao.readFnames(b.getTnum());
            String fs = pldao.readFsizes(b.getTnum());
