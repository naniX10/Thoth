...
@Service("pQsrv")
public class ProductQServiceImpl implements ProductService{

    @Autowired private ProductDAO pQdao;
    ...
    public List<Product> readProduct(String cp) {
        ...
        return pQdao.selectProduct(snum);
    }
    // 상품 조회 - 전체 상품 갯수 구하기
    @Override
    public int countProduct(){
        return pQdao.selectCountProduct();
    }
    ...
    public List<Product> readProduct(String cp, String ftype, String fkey) {
        ...
        return pQdao.findSelectProduct(params);
    }
    ...
    public int countProduct(String ftype, String fkey) {
        ...
        params.put("ftype",ftype);
        params.put("fkey",fkey);
        return pQdao.selectCountProduct(params);
    }

    // 상품 상세 조회
    @Override
    public Product readOneProduct(String tnum) {
        return pQdao.selectOneProduct(tnum);
    }
    ...
    public boolean newProduct(Product p, BookImage b, MultipartFile[] img){
        ...
        if(pQdao.insertProduct(p,b)>0) isInserted = true;
        //pQdao.insertProduct(p,b);
        return isInserted;
    }
    ...
    public Product removeProduct(String tnum) {
        Product p = pQdao.selectOneProduct(tnum); // 지우고자 하는 상품 분류코드
        pQdao.deleteProduct(tnum);
        return p;
    }
    ...
    public BookImage readOneImage(String tnum) {
        return pQdao.selectOneImage(tnum);
    }

    // 상품 삭제 - 이미지
    @Override
    public BookImage removeImage(String tnum) {
        BookImage b = pQdao.selectOneImage(tnum); // 지우고자 하는 상품 분류코드
        pQdao.deleteImage(tnum);
        return b;
    }
    ...
    public void modifyImage(BookImage b, MultipartFile[] img) {
        ...
        if(imgutil.checkImageFiles(img)) {
            ...
            String fn = pQdao.readFnames(b.getTnum());
            String fs = pQdao.readFsizes(b.getTnum());
