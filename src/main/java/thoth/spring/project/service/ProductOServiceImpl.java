...
public class ProductOServiceImpl implements ProductService{
    ...
    @Autowired private ProductDAO pOdao;
    ...
    public List<Product> readProduct(String cp) {
        ...
        return pOdao.selectProduct(snum);
    }
    // 상품 조회 - 전체 상품 갯수 구하기
    @Override
    public int countProduct(){
        return pOdao.selectCountProduct();
    }
    ...
    public List<Product> readProduct(String cp, String ftype, String fkey) {
        ...
        return pOdao.findSelectProduct(params);
    }
    ...
    public int countProduct(String ftype, String fkey) {
        ...
        params.put("ftype",ftype);
        params.put("fkey",fkey);
        return pOdao.selectCountProduct(params);
    }

    // 상품 상세 조회
    @Override
    public Product readOneProduct(String tnum) {
        return pOdao.selectOneProduct(tnum);
    }
    ...
    public boolean newProduct(Product p, BookImage b, MultipartFile[] img){
        ...
        if(pOdao.insertProduct(p,b)>0) isInserted = true;
        //pOdao.insertProduct(p,b);
        return isInserted;
    }
    ...
    public Product removeProduct(String tnum) {
        Product p = pOdao.selectOneProduct(tnum); // 지우고자 하는 상품 분류코드
        pOdao.deleteProduct(tnum);
        return p;
    }
    ...
    public BookImage readOneImage(String tnum) {
        return pOdao.selectOneImage(tnum);
    }

    // 상품 삭제 - 이미지
    @Override
    public BookImage removeImage(String tnum) {
        BookImage b = pOdao.selectOneImage(tnum); // 지우고자 하는 상품 분류코드
        pOdao.deleteImage(tnum);
        return b;
    }
    ...
    public void modifyImage(BookImage b, MultipartFile[] img) {
        ...
        if(imgutil.checkImageFiles(img)) {
            ...
            String fn = pOdao.readFnames(b.getTnum());
            String fs = pOdao.readFsizes(b.getTnum());
