...
@Service("pRsrv")
public class ProductRServiceImpl implements ProductService{

    @Autowired private ProductDAO pRdao;
    ...
    public List<Product> readProduct(String cp) {
        ...
        return pRdao.selectProduct(snum);
    }
    // 상품 조회 - 전체 상품 갯수 구하기
    @Override
    public int countProduct(){
        return pRdao.selectCountProduct();
    }
    ...
    public List<Product> readProduct(String cp, String ftype, String fkey) {
        ...
        return pRdao.findSelectProduct(params);
    }
    ...
    public int countProduct(String ftype, String fkey) {
        ...
        params.put("ftype",ftype);
        params.put("fkey",fkey);
        return pRdao.selectCountProduct(params);
    }

    // 상품 상세 조회
    @Override
    public Product readOneProduct(String tnum) {
        return pRdao.selectOneProduct(tnum);
    }
    ...
    public boolean newProduct(Product p, BookImage b, MultipartFile[] img){
        ...
        if(pRdao.insertProduct(p,b)>0) isInserted = true;
        //pRdao.insertProduct(p,b);
        return isInserted;
    }
    ...
    public Product removeProduct(String tnum) {
        Product p = pRdao.selectOneProduct(tnum); // 지우고자 하는 상품 분류코드
        pRdao.deleteProduct(tnum);
        return p;
    }
    ...
    public BookImage readOneImage(String tnum) {
        return pRdao.selectOneImage(tnum);
    }

    // 상품 삭제 - 이미지
    @Override
    public BookImage removeImage(String tnum) {
        BookImage b = pRdao.selectOneImage(tnum); // 지우고자 하는 상품 분류코드
        pRdao.deleteImage(tnum);
        return b;
    }
    ...
    public void modifyImage(BookImage b, MultipartFile[] img) {
        ...
        if(imgutil.checkImageFiles(img)) {
            ...
            String fn = pRdao.readFnames(b.getTnum());
            String fs = pRdao.readFsizes(b.getTnum());
