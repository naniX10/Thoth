...
@Service("pNsrv")
public class ProductNServiceImpl implements ProductService{

    @Autowired private ProductDAO pNdao;
    ...
    public List<Product> readProduct(String cp) {
        ...
        return pNdao.selectProduct(snum);
    }
    // 상품 조회 - 전체 상품 갯수 구하기
    @Override
    public int countProduct(){
        return pNdao.selectCountProduct();
    }
    ...
    public List<Product> readProduct(String cp, String ftype, String fkey) {
        ...
        return pNdao.findSelectProduct(params);
    }
    ...
    public int countProduct(String ftype, String fkey) {
        ...
        params.put("ftype",ftype);
        params.put("fkey",fkey);
        return pNdao.selectCountProduct(params);
    }

    // 상품 상세 조회
    @Override
    public Product readOneProduct(String tnum) {
        return pNdao.selectOneProduct(tnum);
    }
    ...
    public boolean newProduct(Product p, BookImage b, MultipartFile[] img){
        ...
        if(pNdao.insertProduct(p,b)>0) isInserted = true;
        //pNdao.insertProduct(p,b);
        return isInserted;
    }
    ...
    public Product removeProduct(String tnum) {
        Product p = pNdao.selectOneProduct(tnum); // 지우고자 하는 상품 분류코드
        pNdao.deleteProduct(tnum);
        return p;
    }
    ...
    public BookImage readOneImage(String tnum) {
        return pNdao.selectOneImage(tnum);
    }

    // 상품 삭제 - 이미지
    @Override
    public BookImage removeImage(String tnum) {
        BookImage b = pNdao.selectOneImage(tnum); // 지우고자 하는 상품 분류코드
        pNdao.deleteImage(tnum);
        return b;
    }
    ...
    public void modifyImage(BookImage b, MultipartFile[] img) {
        ...
        if(imgutil.checkImageFiles(img)) {
            ...
            String fn = pNdao.readFnames(b.getTnum());
            String fs = pNdao.readFsizes(b.getTnum());
