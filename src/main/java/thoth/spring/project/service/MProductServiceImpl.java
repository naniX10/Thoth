...
import thoth.spring.project.dao.MProductDAO;
...
public class MProductServiceImpl implements MProductService{
    ...
    @Autowired private MProductDAO pmdao;
    @Autowired private ImgUploadUtil imgutil;

    /* ch */

    @Override
    public List<Product> mreadProduct(String cp) {
        int snum = (Integer.parseInt(cp)-1)*20; // 10개 게시물 출력
        return pmdao.mselectProduct(snum);
    }

    @Override
    public int mcountProduct() {
        return pmdao.mselectCountProduct();
    }

    @Override
    public List<Product> mreadProduct(String cp, String findtype, String findkey) {
        int snum = 10*(Integer.parseInt(cp)-1); // 10개 게시물 고정
        Map<String, Object> params = new HashMap<>();
        params.put("snum",snum);
        params.put("ftype",findtype);
        params.put("fkey",findkey);
        return pmdao.mfindSelectProduct(params);
    }

    @Override
    public int mcountProduct(String findtype, String findkey) {
        Map<String,Object> params = new HashMap<>();
        params.put("ftype",findtype);
        params.put("fkey",findkey);
        return pmdao.mselectCountProduct(params);
    }

    @Override
    public boolean mnewProduct(Product p, BookImage b, MultipartFile[] img) {
        // 식별코드 생성
        ...
        if(pmdao.minsertProduct(p,b)>0) isInserted = true;
        //pdao.insertProduct(p,b);
        return isInserted;
    }

    @Override
    public Product mreadOneProduct(String tnum) {
        return pmdao.mselectOneProduct(tnum);
    }

    @Override
    public BookImage mreadOneImage(String tnum) {
        return pmdao.mselectOneImage(tnum);
    }

    @Override
    public Product mremoveProduct(String tnum) {
        Product p = pmdao.mselectOneProduct(tnum); // 지우고자 하는 상품 분류코드
        pmdao.mdeleteProduct(tnum);
        return p;
    }

    @Override
    public BookImage mremoveImage(String tnum) {
        BookImage b = pmdao.mselectOneImage(tnum); // 지우고자 하는 상품 분류코드
        pmdao.mdeleteImage(tnum);
        return b;
    }

    @Override
    public void mmodifyProduct(Product p) {
        pmdao.mupdateProduct(p);
    }

    @Override
    public void mmodifyImage(BookImage b, MultipartFile[] img) {
        ...
        if(imgutil.checkImageFiles(img)) {
            ...
            String fn = pmdao.readFnames(b.getTnum());
            String fs = pmdao.readFsizes(b.getTnum());
