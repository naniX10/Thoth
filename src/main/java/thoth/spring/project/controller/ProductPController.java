...
public class ProductPController {
    ...
    public ModelAndView plist(ModelAndView mv, String cp) {
        ...
        mv.addObject("pds",pPsrv.readProduct(cp));   // 상품 조회
        mv.addObject("pcnt",pPsrv.countProduct());   // 상품 전체 수 구하기
        return mv;
    }
    // 리스트 - 검색
    ...
    public ModelAndView find(ModelAndView mv, String cp, String findtype, String findkey){
        mv.setViewName("product/plist.tiles");
        mv.addObject("pds",pPsrv.readProduct(cp,findtype,findkey));  // 검색
        mv.addObject("pcnt",pPsrv.countProduct(findtype,findkey));   // 검색된 상품 수 구하기
    ...
    public ModelAndView pview(String tnum, ModelAndView mv) {
        ...
        mv.addObject("p", pPsrv.readOneProduct(tnum));   // tnum 상품 정보 조회
        mv.addObject("b",pPsrv.readOneImage(tnum)); // 이미지 조회
        return mv;
    }
    ...
    public String premove(String tnum){
        Product p = pPsrv.removeProduct(tnum); // 상품 삭제
        BookImage b = pPsrv.removeImage(tnum); // 상품 이미지 삭제
    ...
    public ModelAndView pupdate(ModelAndView mv, String tnum)
    {
        ...
        mv.addObject("p",pPsrv.readOneProduct(tnum));
        mv.addObject("b",pPsrv.readOneImage(tnum));
        return mv;
    }
    ...
    public String pupdateok(Product p,BookImage b, MultipartFile[] img){
        pPsrv.modifyProduct(p);
        if(b.getTodie()!=""){
            pPsrv.modifyImage(b,img);
        }
        return "redirect:/product/plist";
    }
