...
$('#lsavePbtn').on('click',function(){
    if($('#tnum').val()=='') alert('분류코드를 입력해주세요.');
        ...
        const frm = $('#prdfrm');
        frm.attr('action','/product/lpwrite');
...
$('#llistPbtn').on('click',function(){
    location.href='/product/lplist';
});

$('#ldelPbtn').on('click',function(){
    alert('상품이 삭제되었습니다.');
    location.href='/product/lpremove?tnum='+$('#tnum').val();
});

// 상품 수정 버튼 클릭 이벤트
$('#lmodPbtn').on('click',function(){
    location.href = '/product/lpupdate?tnum='+$('#tnum').val();
});
...
$('#lsavePbtn').on('click',function(){
    if(grecaptcha.getResponse()=='') alert('자동가입방지를 입력해주세요.');
        ...
        const frm = $('#modprdfrm');
        frm.attr('action','/product/lpupdate');
        frm.attr('method','post');
        frm.attr('enctype','multipart/form-data');
        frm.submit();
        alert('상품이 수정되었습니다.');
