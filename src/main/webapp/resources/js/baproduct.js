...
$('#basavePbtn').on('click',function(){
    if($('#tnum').val()=='') alert('분류코드를 입력해주세요.');
        ...
        const frm = $('#prdfrm');
        frm.attr('action','/product/bapwrite');
...
$('#balistPbtn').on('click',function(){
    location.href='/product/baplist';
});

$('#badelPbtn').on('click',function(){
    alert('상품이 삭제되었습니다.');
    location.href='/product/bapremove?tnum='+$('#tnum').val();
});

// 상품 수정 버튼 클릭 이벤트
$('#bamodPbtn').on('click',function(){
...
$('#basavePbtn2').on('click',function(){
    if(grecaptcha.getResponse()=='') alert('자동가입방지를 입력해주세요.');
        ...
        const frm = $('#modprdfrm');
        frm.attr('action','/product/bapupdate');
        frm.attr('method','post');
        frm.attr('enctype','multipart/form-data');
        frm.submit();
        alert('상품이 수정되었습니다.');
...
// 상품 등록 버튼 클릭 이벤트 : 상품 등록 페이지로 이동
$('#babuy2Pbtn').on('click',function(){
    location.href = '/product/baporder?tnum='+$('#tnum').val();
    // location.href = '/product/pupdate?tnum='+$('#tnum').val();
})

// 주문완료2
$('#baorderPbtn').on('click',function(){
    if($('#address').val()=='') alert('주소를 입력해주세요.');
    else if(grecaptcha.getResponse()=='') alert('자동가입방지를 입력해주세요.');
    else {
        const frm = $('#babuyprdfrm');
        frm.attr('action','/product/baporder');
        frm.attr('method','post');
        frm.attr('enctype','multipart/form-data');
        frm.submit();
        alert('주문이 완료되었습니다.');
