...
$('#msavePbtn').on('click',function(){
    if($('#tnum').val()=='') alert('분류코드를 입력해주세요.');
        ...
        const frm = $('#prdfrm');
        frm.attr('action','/product/mpwrite');
...
$('#mlistPbtn').on('click',function(){
    location.href='/product/mplist';
});

$('#mdelPbtn').on('click',function(){
    alert('상품이 삭제되었습니다.');
    location.href='/product/mpremove?tnum='+$('#tnum').val();
});

// 상품 수정 버튼 클릭 이벤트
$('#mmodPbtn').on('click',function(){
    location.href = '/product/mpupdate?tnum='+$('#tnum').val();
});
...
$('#msavePbtn').on('click',function(){
    if(grecaptcha.getResponse()=='') alert('자동가입방지를 입력해주세요.');
        ...
        const frm = $('#modprdfrm');
        frm.attr('action','/product/mpupdate');
        frm.attr('method','post');
        frm.attr('enctype','multipart/form-data');
        frm.submit();
        alert('상품이 수정되었습니다.');
