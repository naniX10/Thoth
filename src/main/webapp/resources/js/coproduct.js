...
$('#cosavePbtn').on('click',function(){
    if($('#tnum').val()=='') alert('분류코드를 입력해주세요.');
        ...
        const frm = $('#prdfrm');
        frm.attr('action','/product/copwrite');
...
$('#colistPbtn').on('click',function(){
    location.href='/product/coplist';
});

$('#codelPbtn').on('click',function(){
    alert('상품이 삭제되었습니다.');
    location.href='/product/copremove?tnum='+$('#tnum').val();
});

// 상품 수정 버튼 클릭 이벤트
$('#comodPbtn').on('click',function(){
...
$('#cosavePbtn2').on('click',function(){
    if(grecaptcha.getResponse()=='') alert('자동가입방지를 입력해주세요.');
        ...
        const frm = $('#modprdfrm');
        frm.attr('action','/product/copupdate');
        frm.attr('method','post');
        frm.attr('enctype','multipart/form-data');
        frm.submit();
        alert('상품이 수정되었습니다.');
...
// 상품 등록 버튼 클릭 이벤트 : 상품 등록 페이지로 이동
$('#cobuy2Pbtn').on('click',function(){
    location.href = '/product/porder2?tnum='+$('#tnum').val();
    // location.href = '/product/pupdate?tnum='+$('#tnum').val();
})

// 주문완료2
$('#coorderPbtn').on('click',function(){
    if($('#address').val()=='') alert('주소를 입력해주세요.');
    else if(grecaptcha.getResponse()=='') alert('자동가입방지를 입력해주세요.');
    else {
        const frm = $('#cobuyprdfrm');
        frm.attr('action','/product/coporder');
        frm.attr('method','post');
        frm.attr('enctype','multipart/form-data');
        frm.submit();
        alert('주문이 완료되었습니다.');
